...
from fastapi import APIRouter, BackgroundTasks, Depends, Header, HTTPException, Query, UploadFile, File, Form, status
...
from services.storage import get_signed_url, upload_audio_to_storage
from services.tts import DEFAULT_VOICE
...
async def create_audio_job(
    ...
    background_tasks.add_task(run_audio_job_sync_fallback, job.id, data.language)
    return models.AudioJobCreateResp(job_id=str(job.id), status="queued")


def run_audio_job_sync_fallback(job_id: UUID, language: str):
    import asyncio
    from services.worker import process_audio_job_async
    asyncio.run(process_audio_job_async(None, str(job_id)))
...
def generate_audio_legacy(
    ...
    audio_bytes = generate_audio_from_text(prompt=data.prompt, language=data.language)
...
@router.post("/voices", response_model=models.VoiceProfileResp)
async def upload_voice_profile(
    name: str = Form(..., description="Tên giọng (VD: Giọng của tôi)"),
    sample: UploadFile = File(..., description="File audio 3-5 giây (.wav, .mp3)"),
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user),
):
    if not name.strip():
    ...
    if len(content) < 1024:
        raise HTTPException(status_code=400, detail="File audio quá ngắn, cần ít nhất 3-5 giây.")

    filename = f"voice_{current_user.id}_{int(time.time())}_{sample.filename or 'sample.wav'}"
    storage_url = upload_audio_to_storage(content, filename)

    profile = models.VoiceProfile(
        user_id=current_user.id,
        name=name.strip(),
        sample_url=storage_url,
        is_active=True,
    )
    db.add(profile)
    db.commit()
    ...
    return models.VoiceProfileResp(
        id=str(profile.id),
        name=profile.name,
        sample_url=storage_url,
...
@router.get("/voices", response_model=list[models.VoiceProfileResp])
def list_voice_profiles(
    ...
    profiles = (
        db.query(models.VoiceProfile)
        .filter(models.VoiceProfile.user_id == current_user.id)
        .order_by(models.VoiceProfile.created_at.desc())
        .all()
    )
    return [
        models.VoiceProfileResp(
            id=str(p.id),
            name=p.name,
